"""vlmctx tail -- last N lines/pages/frames."""
...
def tail_cmd(
    ...
    n: Annotated[int, typer.Option("-n", help="Number of lines")] = 10,
    level: Annotated[int, typer.Option("--level", "-l", help="Processing level")] = 1,
) -> None:
    """Last N lines/pages of a file (like tail)."""
    ...
    for file_path in paths:
        ...
        lines = all_lines[-n:]
        output = "\n".join(lines)
        start_line = max(1, len(all_lines) - n + 1)
        ...
        if use_rich:
            ...
            subtitle = Text()
            subtitle.append(f"lines {start_line}-{len(all_lines)}", style="dim")
