...
class DocumentRasterize(Encoder):
    ...
    def encode(self, path: Path, **kwargs: Any) -> Iterable[Message]:
        ...
        page_iter = rasterize_pages(path, max_width, max_pages)
        page_idx = 0
        any_pages = False
...
class DocumentRasterizeText(Encoder):
    ...
    def encode(self, path: Path, **kwargs: Any) -> Iterable[Message]:
        ...
        page_texts: list[str] = extract_page_texts(path, max_pages)
        page_iter = rasterize_pages(path, max_width, max_pages)
        page_idx = 0
        any_pages = False
...
def extract_page_texts(path: Path, max_pages: Optional[int]) -> list[str]:
