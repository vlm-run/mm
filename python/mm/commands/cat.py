...
# Track total decode window (ms) across files: sum of each LLM call's gen_duration_ms.
_total_gen_duration_ms: float = 0.0
...
def _extract(path: Path, opts: CatOpts) -> tuple[str, RunResult | None]:
    ...
    """
    global _was_cached, _total_token_cost, _total_completion_tokens, _total_gen_duration_ms
    kind = file_kind(path)
    ...
    if run.llm_usage is not None:
        completion = run.llm_usage.get("completion_tokens", 0.0)
        window_ms = run.llm_usage.get("gen_duration_ms", 0.0)
        if completion > 0 and window_ms > 0:
            with _cost_lock:
                _total_completion_tokens += completion
                _total_gen_duration_ms += window_ms
