...
class MmDatabase:
    ...
    def upsert_files(
        ...
        *,
        ...
        Thin adapter over :meth:`upsert_records`: it converts the Arrow
        scan table to row dicts, tags every row with ``session_id`` (when
        supplied) and the ``{relative_path: ref_id}`` mapping in ``refs``,
        then delegates the write. Rows without a supplied ref get a fresh
        :func:`mm.refs.make_ref_id`.
        """
        records = scanner_table.to_pylist()
        if session_id is not None:
            ...
            for rec in records:
                ...
                if rel_path in refs:
                    rec["ref_id"] = refs[rel_path]
        return self.upsert_records(records, root=root, scanner=scanner)
    ...
    def upsert_records(
        ...
        *,
        ...
        """Persist :meth:`mm.Context.to_records` output to the ``files`` table.

        This is the storage-side counterpart to the library's
        storage-agnostic export: the :class:`mm.Context` never writes to a
        database, it hands back plain dict records, and the caller (the mm
        CLI, here) writes them to its chosen backend. Records produced with
        ``to_records(refs=True)`` carry ``session_id``/``ref_id`` columns;
        a row tagged with a session but missing a ref gets a fresh
        :func:`mm.refs.make_ref_id`.

        On conflict ``session_id`` is overwritten but ``ref_id`` is
        preserved via ``COALESCE(files.ref_id, excluded.ref_id)`` while the
        row stays in the same session — once tagged, a ref is stable for
        the lifetime of the row.

        Args:
            records: Row dicts from :meth:`mm.Context.to_records`, each with
                the scan-metadata columns (``path``, ``name``, ``size`` …)
                and optionally ``session_id``/``ref_id``.
            root: Directory the records were scanned from; used to build the
                absolute ``uri`` for each row.
            scanner: Optional pre-built ``Scanner`` reused for content
                extraction; one is built from ``root`` when not supplied.

        Returns:
            Total ``files`` row count after the write.
        """
        ...
        now = now_us()
        root_s = str(root)
        has_session = any(rec.get("session_id") is not None for rec in records)
        ...
        for rec in records:
            ...
            if session_id is None:
                ...
                ref_id = rec.get("ref_id") or make_ref_id(kind)
            ...
            rows.append(
                (
                    ...
                    rec.get("ext") or "",
                    rec.get("size") or 0,
                    _to_us(rec.get("modified")),
                    _to_us(rec.get("created")),
                    rec.get("mime") or "",
                    kind,
                    int(bool(rec.get("is_binary"))),
                    rec.get("depth") or 0,
                    f"{root_s}/{parent}" if parent else root_s,
                    rec.get("width"),
                    rec.get("height"),
                    f"{phash:016x}" if phash is not None else None,
                    session_id,
                    ref_id,
                    now,
                )
        ...
        if not scanner or not isinstance(scanner, Scanner):
            ...
            scanner.scan()
        for row, rel_path in zip(rows, rel_paths):
            fill_metadata(self, row[0], Path(row[0]), scanner, rel_path=rel_path)
    ...
    def resolve(
        ...
        *,
        ...
        """Resolve a ref to its ``files`` row.

        Accepts a global ``<session_id>/<ref_id>`` ref, or a bare ref id
        together with ``default_session``. Returns the row dict, or
        ``None`` on a miss. This is the store-side resolver the library
        used to expose as ``Context.resolve``/``Context.get`` (now removed
        — cross-session lookups are the caller's responsibility).
        """
        from mm.refs import GlobalRef

        if "/" in global_ref:
            parsed = GlobalRef.parse(global_ref)
            sid, rid = parsed.session_id, parsed.ref_id
        elif default_session is not None:
            sid, rid = default_session, global_ref
        else:
            raise ValueError(
                f"ambiguous ref {global_ref!r}: pass a global '<session_id>/<ref_id>' "
                "or supply default_session=..."
            )
        return self.get_file_by_ref(sid, rid)
