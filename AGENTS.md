...
- **Incremental writes**: `upsert_files` snapshots `(modified, size, content_hash)` before upserting and re-extracts only new/changed rows via `Scanner.extract_metadata_batch` (rayon, GIL released). `ensure_metadata` is O(1) — single-file `scan_one` + `extract_metadata_one`, never a directory walk.
