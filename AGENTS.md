...
│   │   ├── audio.py            # base64, transcribe, gemini-native
│   │   ├── document/
...
- `audio` → `cat audio.mp3` (Whisper transcript; use `-p base64` or `-p gemini-native` for LLM description)
...
- `mm cat file -m accurate` — LLM-generated caption/description (image/video/PDF); audio: Whisper transcript only unless using `-p base64` or `-p gemini-native`; passthrough for code/text/docx/pptx
...
- **cat accurate**: LLM-powered descriptions via OpenAI-compatible
  ...
  Audio → Whisper transcript only (default `transcribe` encoder suppresses LLM; use `-p base64` or `-p gemini-native` for LLM description). PDFs → page-text → LLM markdown
