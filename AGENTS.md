...
│   │   ├── audio.py            # native, transcript, gemini-native
│   │   ├── document/
...
- **cat accurate**: LLM-powered descriptions via OpenAI-compatible
  ...
  Audio → Whisper transcript only (default `transcript` encoder suppresses LLM; use `-p native` or `-p gemini-native` for LLM description). PDFs → page-text → LLM markdown
