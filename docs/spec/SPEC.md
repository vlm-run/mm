...
│   ├── [x] Document extraction via pypdfium2 (PDF) / anydoc (DOCX/PPTX/XLSX/ODS/ODT/ODP → markdown, fast) / libreoffice-rs (office → PDF, accurate)
...
- documents: (PDFs, Word documents, etc.)
    - pypdfium2 for PDF text extraction, anydoc for Office formats (→ GitHub-Flavored Markdown, fast mode), libreoffice-rs for office → PDF conversion (accurate mode)
