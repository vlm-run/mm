...
`mm cat meeting.mp3 -p base64 -m accurate` runs the full pipeline: audio extraction and transcription or LLM summarization. The default `transcribe` encoder is encode-only — use `-p base64` or `-p gemini` for LLM description.
</details>
