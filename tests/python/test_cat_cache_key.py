...
The extractions cache fragment for `mm cat` is built from the user's overrides. Dict-
...
from mm.cat_utils.base_utils import override_extra


def test_strategy_opts_order_independence():
    """Same overrides, different CLI order → identical cache fragment."""
    a = override_extra({"strategy_opts": {"max_width": 768, "fps": 5}}, {}, {})
    b = override_extra({"strategy_opts": {"fps": 5, "max_width": 768}}, {}, {})
...
def test_distinct_overrides_still_diverge():
    ...
    a = override_extra({"strategy_opts": {"max_width": 768}}, {}, {})
    b = override_extra({"strategy_opts": {"max_width": 1024}}, {}, {})
    assert a != b
