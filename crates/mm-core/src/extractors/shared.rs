...
use crate::extract::{ExtractError, MetadataRecord};

// ---------------------------------------------------------------------------
// Symphonia — MP3, WAV, FLAC, AAC, OGG/Vorbis, Opus
// ---------------------------------------------------------------------------

pub fn extract_symphonia(path: &Path) -> Result<MetadataRecord, ExtractError> {
...
pub fn extract_mp4(path: &Path) -> Result<MetadataRecord, ExtractError> {
...
pub fn extract_matroska(path: &Path) -> Result<MetadataRecord, ExtractError> {
    let file = File::open(path)?;
