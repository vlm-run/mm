...
|--:|:---------------------------------|----------:|-----------:|:------:|--------:|
| 1 | `video-clips`                    |        37 |         35 |    -5% |   1.06× |
| 2 | `video-chunks`                   |     2,145 |      1,806 |   -16% |   1.19× |
| 3 | `video-keyframes`                |     2,278 |      1,385 |   -39% |   1.64× |
| 4 | `video-summary`                  |     5,306 |      3,620 |   -32% |   1.47× |
| 5 | `video-frames`                   |     4,270 |      3,738 |   -12% |   1.14× |
| 6 | `video-mosaic`                   |     5,126 |      4,834 |    -6% |   1.06× |
| 7 | `video-shots`                    |    16,074 |     13,382 |   -17% |   1.20× |
| 8 | `video-shot-mosaic`              |    22,770 |     20,753 |    -9% |   1.10× |
| 9 | `video-captions`                 |    78,284 |     76,082 |    -3% |   1.03× |
|10 | `video-transcript`               |    78,898 |     75,856 |    -4% |   1.04× |
|11 | `video-clips-w-transcript`       |    79,392 |     76,471 |    -4% |   1.04× |
|12 | `video-keyframes-w-transcript`   |    80,489 |     77,135 |    -4% |   1.04× |
|13 | `video-frames-w-transcript`      |    81,716 |     74,035 |    -9% |   1.10× |
|14 | `video-summary-w-transcript`     |    82,636 |     74,427 |   -10% |   1.11× |
|15 | `video-mosaic-w-transcript`      |    82,851 |     79,502 |    -4% |   1.04× |
|16 | `video-shots-w-transcript`       |    92,221 |     79,293 |   -14% |   1.16× |
|17 | `video-shot-mosaic-w-transcript` |   101,647 |     84,699 |   -17% |   1.20× |
...
(bundled seeks for `video-shots*`). The `-w-transcript` rows show a smaller relative
...
`video-shot-mosaic-w-transcript` (101.6s → 84.7s) where visual ≈ Whisper, so the
...
|:---------------------------------|----------:|---------:|:------:|--------:|
| `video-transcript`               |    78,898 |        0 | -100% | ∞       |
| `video-captions`                 |    78,284 |        8 | -100% | 9,786×  |
| `video-clips`                    |        37 |       29 |  -22% |   1.28× |
| `video-clips-w-transcript`       |    79,392 |       30 | -100% | 2,646×  |
| `video-summary-w-transcript`     |    82,636 |      215 | -100% |   384×  |
| `video-summary`                  |     5,306 |      219 |  -96% |    24×  |
| `video-mosaic`                   |     5,126 |    1,056 |  -79% |   4.85× |
| `video-mosaic-w-transcript`      |    82,851 |    1,094 |  -99% |    76×  |
| `video-keyframes`                |     2,278 |    1,366 |  -40% |   1.67× |
| `video-keyframes-w-transcript`   |    80,489 |    1,374 |  -98% |    59×  |
| `video-chunks`                   |     2,145 |    1,748 |  -19% |   1.23× |
| `video-frames-w-transcript`      |    81,716 |    3,695 |  -95% |    22×  |
| `video-frames`                   |     4,270 |    3,705 |  -13% |   1.15× |
| `video-shots-w-transcript`      |    92,221 |    9,007 |  -90% |    10×  |
| `video-shots`                    |    16,074 |    9,115 |  -43% |   1.76× |
| `video-shot-mosaic-w-transcript`|   101,647 |   15,578 |  -85% |   6.5×  |
| `video-shot-mosaic`              |    22,770 |   15,467 |  -32% |   1.47× |
...
On a real pipeline that runs `video-mosaic`, then `video-shots`, then
`video-shot-mosaic` against the same file, total wall time drops from
**44.0s → 25.6s** (1.7×) without transcripts and from **207.3s → 25.7s** (8.1×) with
transcripts — Whisper runs exactly once.

## Observations

1. **`video-shots` pays for the bundled seek**: `13.4s → 9.1s` warm reflects both the
...
2. **`video-keyframes` is purely `frame.reformat`**: 2.28s → 1.39s (-39%). No caching
...
3. **`video-mosaic` pre/post warm is dominated by scene-detect**: 5.13s → 4.83s cold
...
4. **`-w-transcript` wall time floor is `max(visual, whisper)`**: post-P0,
   `video-shot-mosaic-w-transcript` cold is `max(20.7s, 76s) ≈ 84.7s` (about 8s of
...
- **Forward-decode for dense extraction** (`video-frames`, `video-chunks`) — replace
...
| `TestCacheCrossEncoderReuse`       | Running `video-mosaic` then `video-shots` against the same file does not grow either cache.       |
